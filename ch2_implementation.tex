...
The full list of compilation options (\code{build} script usage) can be found in the Decomposition GitLab repository\footref{Footnote:decomposition-project-gitlab-url}. \\
...
In this section, each part of the project's structure will be briefly described. First, unit tests will be detailed followed by the implementation of the Crout method (introduced in Section~\ref{Subsection:theory-LU-decomposition-crout-method}), along with the Iterative Crout method (introduced in Section~\ref{Subsection:theory-LU-decomposition-numerical-method}). Finally, the last part focuses on the benchmark implementation that aims to put into perspective the performance differences between the direct and iterative approach. \\
...
As the testing infrastructure was mostly adopted from TNL (detailed in Section 3.1 of the author's bachelor thesis \emph{Formats for storage of sparse matrices on GPU} \cite{Cejka2020}) it will only be described briefly.
...
Since the implementation of the Crout method in the Decomposition project differs only slightly from the pseudocode shown in Listing~\ref{Listing:theory-LU-decomposition-crout-method-pseudocode}, the code will not be shown in this thesis - it can be found in the project's GitLab repository\footref{Footnote:decomposition-project-gitlab-url}. \\
